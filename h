...
if [ -z "$1" ]; then
    echo "============================================="
    echo "  СИСТЕМА УЧЁТА УСПЕВАЕМОСТИ ВУЗ"
    echo "============================================="
    echo ""
    ...
    echo ""
    echo "================== ПОДЗАПРОСЫ =================="
    echo "  ./h 110 - отклонение среднего балла"
    echo "  ./h 111 - рейтинг преподавателей"
    echo "  ./h 112 - студенты выше среднего"
    echo "  ./h 113 - статистика по факультетам"
    echo "  ./h 114 - топ-3 студента на факультете"
    echo "  ./h 115 дисциплина - оценки выше среднего"
    echo "  ./h 116 - студенты без троек и двоек"
    echo "  ./h 117 - лучшие студенты ALL"
    echo "  ./h 199 - выполнить все"
    echo ""
    echo "================== ФУНКЦИИ =================="
...
case "$1" in
    ...
    200)
        echo "============================================="
        echo "ФУНКЦИИ АНАЛИТИКИ"
        echo "============================================="
        psql -d $DATABASE -f queries/functions/01_students_above_threshold.sql
        psql -d $DATABASE -f queries/functions/02_grades_by_faculty_season.sql
        psql -d $DATABASE -f queries/functions/03_pivot_table.sql
    ...
    201)
        if [ -z "$2" ]; then
            ...
            psql -d $DATABASE -f queries/functions/04_run_all_functions.sql -v arg1=4.0
        else
            psql -d $DATABASE -f queries/functions/04_run_all_functions.sql -v arg1="$2"
        fi
        ;;
    202)
        echo "============================================="
